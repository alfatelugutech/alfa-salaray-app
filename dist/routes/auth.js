...
router.post("/register", async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || "default-secret", { expiresIn: process.env.JWT_EXPIRES_IN || "7d" });
        res.status(201).json({
            success: true,
