...
function analyzeTrends(attendances) {
    ...
    if (recentPresent > previousPresent)
        return { trend: 'IMPROVING', change: recentPresent - previousPresent };
    if (recentPresent < previousPresent)
        return { trend: 'DECLINING', change: previousPresent - recentPresent };
    return { trend: 'STABLE', change: 0 };
}
function generateSmartRecommendations(attendanceRate, patterns) {
...
function getPerformanceMessage(attendanceRate) {
    if (attendanceRate >= 95)
        return '🌟 Excellent attendance record!';
    if (attendanceRate >= 85)
        return '👍 Good attendance performance';
    if (attendanceRate >= 70)
...
router.get("/dashboard/live", auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get("/analytics/working-hours", auth_1.authenticateToken, async (req, res) => {
    try {
        const { employeeId, startDate, endDate } = req.query;
...
function getProductivityMessage(score) {
    if (score >= 90)
        return '🌟 Outstanding productivity! Keep up the excellent work.';
    if (score >= 75)
        return '👍 Good productivity level. Continue the great work.';
    if (score >= 60)
        return '📈 Productivity is improving. Keep focusing on efficiency.';
