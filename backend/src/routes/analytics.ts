...
const analyticsQuerySchema = Joi.object({
  ...
  groupBy: Joi.string().valid("day", "week", "month", "department", "employee").optional()
});
  ...
  try {
    ...
    if (groupBy === "department") {
      groupedData = attendances.reduce((acc, attendance) => {
        ...
        acc[dept].totalDays++;
        if (attendance.status === 'PRESENT') acc[dept].presentDays++;
        if (attendance.status === 'ABSENT') acc[dept].absentDays++;
        if (attendance.status === 'LATE') acc[dept].lateDays++;
        if (attendance.status === 'HALF_DAY') acc[dept].halfDays++;
        acc[dept].totalHours += Number(attendance.totalHours) || 0;
        if (attendance.isRemote) acc[dept].remoteWorkDays++;
        return acc;
      }, {});
      ...
      groupedData = attendances.reduce((acc, attendance) => {
        ...
        acc[empId].totalDays++;
        if (attendance.status === 'PRESENT') acc[empId].presentDays++;
        if (attendance.status === 'ABSENT') acc[empId].absentDays++;
        if (attendance.status === 'LATE') acc[empId].lateDays++;
        if (attendance.status === 'HALF_DAY') acc[empId].halfDays++;
        acc[empId].totalHours += Number(attendance.totalHours) || 0;
        if (attendance.isRemote) acc[empId].remoteWorkDays++;
        return acc;
      }, {});
    }
...
router.get("/departments", requireHR, async (req: Request, res: Response) => {
  try {
...
router.get("/performance", requireHR, async (req: Request, res: Response) => {
  try {
