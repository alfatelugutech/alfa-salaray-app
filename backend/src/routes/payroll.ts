...
router.put("/:id/process-payment", requireHR, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.post("/bulk-payment", requireHR, async (req: Request, res: Response) => {
  try {
...
router.get("/employee/:employeeId/payment-history", requireHR, async (req: Request, res: Response) => {
  try {
    const { employeeId } = req.params;
    const { page = 1, limit = 10, year } = req.query;
...
router.get("/payment-stats", requireHR, async (req: Request, res: Response) => {
  try {
