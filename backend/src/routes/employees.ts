...
router.get('/analytics/smart-insights', requireHR, async (req: Request, res: Response) => {
  try {
    ...
    const employees = await prisma.employee.findMany({
      where: department ? { departmentId: department as string } : {},
      include: {
        user: true,
        department: true,
        attendances: {
    ...
    const departmentStats = employees.reduce((acc, employee) => {
      ...
      if (insight) {
        if (insight.performance === 'EXCELLENT') acc[dept].excellent++;
        else if (insight.performance === 'GOOD') acc[dept].good++;
        else if (insight.performance === 'NEEDS_IMPROVEMENT') acc[dept].needsImprovement++;
        else acc[dept].poor++;
      }
...
function generateEmployeeRecommendations(employeeInsights: any[], departmentStats: any): string[] {
...
router.put("/:id", requireHR, async (req: Request, res: Response) => {
  try {
    ...
    if (employeeData.employeeId !== undefined) employeeUpdateData.employeeId = employeeData.employeeId;
    ...
    if (employeeData.salary !== undefined) employeeUpdateData.salary = employeeData.salary;
    if (employeeData.status !== undefined) employeeUpdateData.status = employeeData.status;
    if (employeeData.workLocation !== undefined) employeeUpdateData.workLocation = employeeData.workLocation;
